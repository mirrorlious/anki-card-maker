...
导入 Miki 时，选择 **Miki 卡片包**，再到 Miki「智能制卡 → 导入专业制卡包」上传 `.miki-cards.json`。包内仅包含已审核卡片及来源文件名、章节、页码和摘录，不包含 PDF 正文文件、账号或 AI 设置。每包最多 500 张、5 MiB；Miki 中可继续编辑、勾选并选择目标卡组后保存。
